...
# Use lbp features
feature_gray<-read.csv("gray.csv",header = T, as.is = T)
sift_pca<-read.csv("feature_pca100.csv",header = T, as.is = T)
new_feature<-cbind(feature_gray[,-1],sift_pca[,-1])

label<-read.csv("label_train.csv",header = T,as.is = T)
dat<-cbind(label[,2],new_feature)
...
# Tune the model
xgb_params_6 = list(objective="multi:softprob",
                    eta = 0.01,
                    max.depth = 3,
                    eval_metric = "mlogloss",
                    num_class = 3)
...
xgb_6 = xgboost(data = train_matrix, 
                label = train_label,
                params = xgb_params_6,
                nrounds = 100)

# cross validation
xgb_cv_6 = xgb.cv(params = xgb_params_6,
                  data = train_matrix, 
                  nrounds = 100,
                  nfold = 5,
                  showsd = T,
...
xgb_grid_6 = expand.grid(nrounds=c(100,250,500),
                         eta = c(1,0.1,0.01),
...
xgb_trcontrol_6 = trainControl(method = "cv",
                               number = 5,
                               verboseIter = T,
                               returnData = F,
                               returnResamp = "all",
...
xgb_train_6 = train(x=train_matrix, y=train_label,
                    trControl = xgb_trcontrol_6,
                    tuneGrid = xgb_grid_6,
                    method = "xgbTree")
...
## Time for training: 719.99s


head(xgb_train_6$results[with(xgb_train_6$results,order(RMSE)),],5)
# get the best model's parameters
xgb_train_6$bestTune

# best model
bst = xgboost(data=train_matrix,max.depth=8,eta=0.01,nthread=2,nround=500,colsample_bytree=0.5,min_child_weight=2,subsample=1,objective="multi:softprob",eval_metric="mlogloss",num_class=3)
...
## Accuracy: 82.33%
## Parameters: max.depth=8, eta=0.01, nthread=2, nround=500