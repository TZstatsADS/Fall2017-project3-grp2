...
# Use lbp features
feature_lbp<-read.csv("lbp.csv",header = F, as.is = T)
label<-read.csv("label_train.csv",header = T,as.is = T)
dat<-cbind(label[,2],feature_lbp)
...
# Tune the model
xgb_params_4 = list(objective="multi:softprob",
                    eta = 0.01,
                    max.depth = 3,
                    eval_metric = "mlogloss",
                    num_class = 3)
...
xgb_4 = xgboost(data = train_matrix, 
                label = train_label,
                params = xgb_params_4,
                nrounds = 100)

# cross validation
xgb_cv_4 = xgb.cv(params = xgb_params_4,
                  data = train_matrix, 
                  nrounds = 100,
                  nfold = 5,
                  showsd = T,
...
xgb_grid_4 = expand.grid(nrounds=c(100,250,500),
                         eta = c(1,0.1,0.01),
...
xgb_trcontrol_4 = trainControl(method = "cv",
                               number = 5,
                               verboseIter = T,
                               returnData = F,
                               returnResamp = "all",
...
xgb_train_4 = train(x=train_matrix, y=train_label,
...
## Time for training: 168.69s


head(xgb_train_4$results[with(xgb_train_4$results,order(RMSE)),],5)
# get the best model's parameters
xgb_train_4$bestTune

# best model
bst = xgboost(data=train_matrix,max.depth=4,eta=0.1,nthread=2,nround=500,colsample_bytree=0.5,min_child_weight=2,subsample=1,objective="multi:softprob",eval_metric="mlogloss",num_class=3)
